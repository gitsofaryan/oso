...
import type {
  ...
  SystemContext,
} from "@/app/api/v1/osograph/types/context";
 ...
 * @param getOrgId - Extracts the org ID from context and args (may return a Promise)
...
export function withOrgScopedClient<TParent, TArgs>(
  ...
  return async (parent, context, args) => {
    const orgId = await getOrgId({ parent, context, args });
  ...
  return async (parent, context, args) => {
    const resolvedResourceType =
      typeof resourceType === "function"
        ? resourceType({ parent, context, args })
        : resourceType;
    const resourceId = getResourceId({ parent, context, args });
    const { client, permissionLevel, orgId } = await getOrgResourceClient(
      context,
...
>(): Middleware<TParent, GraphQLContext, SystemContext, TArgs, TArgs> {
  return async (_parent, context, args) => {
...
>(
  ...
  return async (_parent, context, args) => {
    if (args.input === undefined) {
      throw new Error("withValidation requires args.input to be defined");
    }
...
export function withLogging<TContext, TArgs>(
  ...
  return async (_parent, context, args) => {
    logger.debug(`[${label}] Starting`, { args });
    return { context, args };
  };
}
